...
extends FilterOutputStream {
    ...
    public void write(byte[] b, int off, int len) throws IOException {
        ...
        if (this.isFrameHeaderBuffered() && !this.isFrameHeaderWriteComplete()) {
            ...
            boolean bl = this.compressionEnabled = this.framePayloadLength >= 250;
        ...
        if ((bytesToDump = len - bytesProcessed) > 0) {
            if (this.compressionEnabled) {
    ...
    private void finalizeWrite() throws IOException {
        if (this.isWriteComplete()) {
            if (this.compressionEnabled) {
                ...
                MysqlxConnection.Compression compressedMessage = MysqlxConnection.Compression.newBuilder().setUncompressedSize(5 + this.framePayloadLength).setClientMessages(Mysqlx.ClientMessages.Type.forNumber(this.xMessageHeader.getMessageType())).setPayload(ByteString.copyFrom((byte[])compressedData)).build();
