...
public class ClassGenerator {
    ...
    public static void generate() {
        if (ViaVersionPlugin.getInstance().isCompatSpigotBuild() || ViaVersionPlugin.getInstance().isProtocolSupport()) {
            try {
                ...
                constructor = (HandlerConstructor)ClassGenerator.toClass(generated).newInstance();
            }
    ...
    public static boolean shouldUseNewHandshakeVersionMethod() {
        try {
            NMSUtil.nms("PacketHandshakingInSetProtocol", "net.minecraft.network.protocol.handshake.PacketHandshakingInSetProtocol").getMethod("getProtocolVersion", new Class[0]);
            return true;
        }
        catch (Exception e) {
            return false;
