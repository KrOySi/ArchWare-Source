...
extends ViaSubCommand {
    ...
    public boolean execute(final ViaCommandSender sender, String[] args) {
        final VersionInfo version = new VersionInfo(System.getProperty("java.version"), System.getProperty("os.name"), Via.getAPI().getServerVersion().lowestSupportedVersion(), Via.getManager().getProtocolManager().getSupportedVersions(), Via.getPlatform().getPlatformName(), Via.getPlatform().getPlatformVersion(), Via.getPlatform().getPluginVersion(), "git-ViaVersion-4.0.1-1.17.1-rc1-SNAPSHOT:4f7e655", Via.getManager().getSubPlatforms());
        ...
        Via.getPlatform().runAsync(new Runnable(){
            ...
            public void run() {
                ...
                try {
                    ...
                    String rawOutput = CharStreams.toString((Readable)new InputStreamReader(con.getInputStream()));
                    ...
                    try {
                        if (con.getResponseCode() < 200 || con.getResponseCode() > 400) {
                            String rawOutput = CharStreams.toString((Readable)new InputStreamReader(con.getErrorStream()));
