...
implements Protocol<C1, C2, S1, S2> {
    ...
    public final void initialize() {
        Preconditions.checkArgument((!this.initialized ? 1 : 0) != 0);
    ...
    protected void registerClientboundChannelIdChanges() {
        ...
        for (ClientboundPacketType packet : (ClientboundPacketType[])this.oldClientboundPacketEnum.getEnumConstants()) {
            ...
            if (mappedPacket == null) {
                Preconditions.checkArgument((boolean)this.hasRegisteredClientbound(State.PLAY, oldId), (Object)("Packet " + packet + " in " + this.getClass().getSimpleName() + " has no mapping - it needs to be manually cancelled or remapped!"));
                continue;
            }
            int newId = mappedPacket.getId();
            if (this.hasRegisteredClientbound(State.PLAY, oldId)) continue;
    ...
    protected void registerServerboundChannelIdChanges() {
        ...
        for (ServerboundPacketType packet : (ServerboundPacketType[])this.newServerboundPacketEnum.getEnumConstants()) {
            ...
            if (mappedPacket == null) {
                Preconditions.checkArgument((boolean)this.hasRegisteredServerbound(State.PLAY, newId), (Object)("Packet " + packet + " in " + this.getClass().getSimpleName() + " has no mapping - it needs to be manually cancelled or remapped!"));
                continue;
            }
            int oldId = mappedPacket.getId();
            if (this.hasRegisteredServerbound(State.PLAY, newId)) continue;
    ...
    private void throwRemapError(Direction direction, State state, int oldId, int newId, InformativeException e) throws InformativeException {
        ...
        Class<Object> class_ = state == State.PLAY ? (direction == Direction.CLIENTBOUND ? this.oldClientboundPacketEnum : this.newServerboundPacketEnum) : (packetTypeClass = null);
