...
extends Gui {
    ...
    private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_) {
        if (ArchWare.moduleManager.getModuleByClass(CrossHair.class).isToggled() || ArchWare.moduleManager.getModuleByClass(CoolFeatureBtw.class).isToggled()) {
            return;
        }
        GameSettings gamesettings = this.mc.gameSettings;
        if (gamesettings.thirdPersonView == 0) {
            if (this.mc.playerController.isSpectator() && this.mc.pointedEntity == null || ArchWare.moduleManager.getModuleByName("NoClip").isToggled()) {
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        if (ArchWare.moduleManager.getModuleByClass(NoScoreboard.class).isToggled()) {
            return;
        }
        ...
        for (Score score1 : collection) {
            ...
            String s3 = "archware.fun";
            GuiIngame.drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 0x60000000);
            GuiIngame.drawRect(l1 - 2, k - 1, l, k, 0x50000000);
            GuiIngame.drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - this.getFontRenderer().FONT_HEIGHT - 2, RenderUtil.getRainbow(500, 5000).hashCode());
            FontUtil.drawString(s3, l1 + i / 2 - FontUtil.getStringWidth(s3) / 2, (double)(k - FontUtil.getFontHeight()) - 1.5, 0x20FFFFFF);
        }
    }
    ...
    private void renderPumpkinOverlay(ScaledResolution scaledRes) {
        if (ArchWare.settingManager.getSettingById("NoRenderPumpkin").getValueBoolean() && ArchWare.moduleManager.getModuleByName("NoRender").isToggled()) {
            return;
        }
        GlStateManager.disableDepth();
    ...
    private void renderPortal(float timeInPortal, ScaledResolution scaledRes) {
        ...
        if (ArchWare.settingManager.getSettingById("NoRenderPortal").getValueBoolean() && ArchWare.moduleManager.getModuleByName("NoRender").isToggled()) {
            return;
        }
        GlStateManager.disableAlpha();
        GlStateManager.disableDepth();
