...
import me.archware.impl.events.EventAnimation;
...
import me.archware.notifications.NotificationRenderer;
...
implements IResourceManagerReloadListener {
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (ArchWare.moduleManager.getModuleByClass(NoHurt.class).isToggled()) {
            return;
        }
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    float f4 = (i >> 1 & 1) * 2 - 1;
                    float f5 = (i >> 2 & 1) * 2 - 1;
                    RayTraceResult raytraceresult = this.mc.world.rayTraceBlocks(new Vec3d(d0 + (double)(f3 *= 0.1f), d1 + (double)(f4 *= 0.1f), d2 + (double)(f5 *= 0.1f)), new Vec3d(d0 - d4 + (double)f3 + (double)f5, d1 - d6 + (double)f4, d2 - d5 + (double)f5));
                    if (ArchWare.moduleManager.getModuleByClass(ViewClip.class).isToggled() || raytraceresult == null || !((d7 = raytraceresult.hitVec.distanceTo(new Vec3d(d0, d1, d2))) < d3)) continue;
                    d3 = d7;
                }
                if (this.mc.gameSettings.thirdPersonView == 2) {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        NotificationRenderer.render(partialTicks, new ScaledResolution(this.mc));
        EventAnimation eventAnimation = new EventAnimation();
        eventAnimation.call();
        this.frameFinish();
    ...
    public static void drawNameplate(FontRenderer fontRendererIn, String str, float x, float y, float z, int verticalShift, float viewerYaw, float viewerPitch, boolean isThirdPersonFrontal, boolean isSneaking, Entity entity) {
        if (ArchWare.moduleManager.getModuleByName("NameTags").isToggled()) {
            return;
        }
        if (entity.getEntityId() == -1337) {
            return;
        }
        if (!ArchWare.moduleManager.getModuleByClass(NameTags.class).isToggled()) {
            ...
            int health = (int)(((EntityPlayer)entity).getHealth() / ((EntityPlayer)entity).getMaxHealth() * 100.0f);
            int i = fontRendererIn.getStringWidth(str + " " + health + "%") / 2;
            GlStateManager.disableTexture2D();
            Tessellator tessellator = Tessellator.getInstance();
            BufferBuilder bufferbuilder = tessellator.getBuffer();
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_COLOR);
            bufferbuilder.pos(-i - 1, -1 + verticalShift, 0.0).color(0.0f, 0.0f, 0.0f, 100.0f).endVertex();
            bufferbuilder.pos(-i - 1, 8 + verticalShift, 0.0).color(0.0f, 0.0f, 0.0f, 100.0f).endVertex();
            bufferbuilder.pos(i + 1, 8 + verticalShift, 0.0).color(0.0f, 0.0f, 0.0f, 100.0f).endVertex();
            bufferbuilder.pos(i + 1, -1 + verticalShift, 0.0).color(0.0f, 0.0f, 0.0f, 100.0f).endVertex();
            tessellator.draw();
            GlStateManager.enableTexture2D();
            GlStateManager.depthMask(true);
            fontRendererIn.drawString(str + (Object)((Object)TextFormatting.GREEN) + " " + health + "%", -fontRendererIn.getStringWidth(str + " " + health + "%") / 2, verticalShift, -1);
            GlStateManager.enableDepth();
            GlStateManager.enableLighting();
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            GlStateManager.popMatrix();
