...
public class Selector {
    ...
    implements JsonDeserializer<Selector> {
        ...
        static ICondition getOrAndCondition(JsonObject json) {
            ...
            if (set.size() == 1) {
                if (json.has("OR")) {
                    return new ConditionOr(Iterables.transform((Iterable)JsonUtils.getJsonArray(json, "OR"), FUNCTION_OR_AND));
                }
                return json.has("AND") ? new ConditionAnd(Iterables.transform((Iterable)JsonUtils.getJsonArray(json, "AND"), FUNCTION_OR_AND)) : Deserializer.makePropertyValue((Map.Entry)set.iterator().next());
            }
            return new ConditionAnd(Iterables.transform((Iterable)set, FUNCTION_PROPERTY_VALUE));
        }
