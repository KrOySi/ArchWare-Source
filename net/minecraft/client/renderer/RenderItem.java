...
implements IResourceManagerReloadListener {
    ...
    protected void renderItemModel(ItemStack stack, IBakedModel bakedmodel, ItemCameraTransforms.TransformType transform, boolean leftHanded) {
        if (!stack.func_190926_b()) {
            ...
            try {
                if (Minecraft.getMinecraft().getRenderManager().options.thirdPersonView == 0 && ArchWare.moduleManager.getModuleByName("MiniItems").isToggled()) {
                    GlStateManager.scale(ArchWare.settingManager.getSettingById("ViewModelSize").getValueNumeric(), ArchWare.settingManager.getSettingById("ViewModelSize").getValueNumeric(), ArchWare.settingManager.getSettingById("ViewModelSize").getValueNumeric());
                }
            }
