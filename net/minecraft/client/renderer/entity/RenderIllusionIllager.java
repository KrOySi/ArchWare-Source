    ...
    public void doRender(EntityMob entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (entity.isInvisible()) {
            ...
            for (int i = 0; i < avec3d.length; ++i) {
                super.doRender(entity, x + avec3d[i].xCoord + (double)MathHelper.cos((float)i + f * 0.5f) * 0.025, y + avec3d[i].yCoord + (double)MathHelper.cos((float)i + f * 0.75f) * 0.0125, z + avec3d[i].zCoord + (double)MathHelper.cos((float)i + f * 0.7f) * 0.025, entityYaw, partialTicks);
            }
        } else {
