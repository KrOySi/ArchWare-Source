...
import me.archware.impl.events.EventPostUpdate;
...
extends AbstractClientPlayer {
    ...
    public boolean startRiding(Entity entityIn, boolean force) {
        ...
        if (entityIn instanceof EntityBoat) {
            ...
            this.prevRotationPitch = entityIn.rotationPitch;
    ...
    public void onUpdate() {
        ...
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
            ...
            if (this.isRiding()) {
                ...
                EventPostUpdate eventPostUpdate = new EventPostUpdate();
                eventPostUpdate.call();
            }
        }
    }
    ...
    private void onUpdateWalkingPlayer() {
        boolean flag1;
        EventPreUpdate eventPreUpdate = new EventPreUpdate(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround, this.getEntityBoundingBox().minY);
        eventPreUpdate.call();
        ...
        if (this.isCurrentViewEntity()) {
            ...
            double d0 = eventPreUpdate.getX() - this.lastReportedPosX;
            double d1 = eventPreUpdate.getMinY() - this.lastReportedPosY;
            double d2 = eventPreUpdate.getZ() - this.lastReportedPosZ;
            double d3 = eventPreUpdate.getYaw() - this.lastReportedYaw;
            double d4 = eventPreUpdate.getPitch() - this.lastReportedPitch;
            ++this.positionUpdateTicks;
            boolean flag2 = d0 * d0 + d1 * d1 + d2 * d2 > 9.0E-4 || this.positionUpdateTicks >= 20;
            boolean bl = flag3 = d3 != 0.0 || d4 != 0.0;
            if (this.isRiding()) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0, this.motionZ, eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
                flag2 = false;
            } else if (flag2 && flag3) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(eventPreUpdate.getX(), eventPreUpdate.getMinY(), eventPreUpdate.getZ(), eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
            } else if (flag2) {
                this.connection.sendPacket(new CPacketPlayer.Position(eventPreUpdate.getX(), eventPreUpdate.getMinY(), eventPreUpdate.getZ(), eventPreUpdate.isOnGround()));
            } else if (flag3) {
                this.connection.sendPacket(new CPacketPlayer.Rotation(eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
            } else if (this.prevOnGround != eventPreUpdate.isOnGround()) {
                this.connection.sendPacket(new CPacketPlayer(eventPreUpdate.isOnGround()));
            }
            if (flag2) {
                this.lastReportedPosX = eventPreUpdate.getX();
                this.lastReportedPosY = eventPreUpdate.getMinY();
                this.lastReportedPosZ = eventPreUpdate.getZ();
                this.positionUpdateTicks = 0;
            }
            if (flag3) {
                this.lastReportedYaw = eventPreUpdate.getYaw();
                this.lastReportedPitch = eventPreUpdate.getPitch();
            }
            this.prevOnGround = eventPreUpdate.isOnGround();
    ...
    public void sendChatMessage(String message) {
        if (!message.startsWith(".")) {
            ...
            ArchWare.commandManager.commands.forEach(command -> {
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        if (ArchWare.moduleManager.getModuleByClass(NoPush.class).isToggled()) {
            return false;
        }
        if (this.noClip) {
            return false;
    ...
    public void setSneak(boolean state) {
        if (this.movementInput != null && !this.sleeping) {
            this.movementInput.sneak = state;
        }
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.isHandActive() && !this.isRiding()) {
            EventSlowdown eventSlowdown = new EventSlowdown(this.movementInput.moveStrafe, this.movementInput.field_192832_b);
            eventSlowdown.call();
        ...
        if (this.capabilities.isFlying && this.isCurrentViewEntity()) {
            if (this.movementInput.sneak) {
                ...
                this.movementInput.field_192832_b = (float)((double)this.movementInput.field_192832_b / 0.3);
