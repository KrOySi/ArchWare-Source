    ...
    private boolean isInventoryFull(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int k : aint = isidedinventory.getSlotsForFace(side)) {
                ...
                if (!itemstack1.func_190926_b() && itemstack1.func_190916_E() == itemstack1.getMaxStackSize()) continue;
                return false;
            }
        } else {
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int i : aint = isidedinventory.getSlotsForFace(side)) {
                if (isidedinventory.getStackInSlot(i).func_190926_b()) continue;
                return false;
            }
        } else {
            int j = inventoryIn.getSizeInventory();
            for (int k = 0; k < j; ++k) {
                if (inventoryIn.getStackInSlot(k).func_190926_b()) continue;
                return false;
            }
        }
        return true;
