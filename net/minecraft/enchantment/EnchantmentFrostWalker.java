    ...
    public static void freezeNearby(EntityLivingBase living, World worldIn, BlockPos pos, int level) {
        if (living.onGround) {
            ...
            for (BlockPos.MutableBlockPos blockpos$mutableblockpos1 : BlockPos.getAllInBoxMutable(pos.add(-f, -1.0, -f), pos.add(f, -1.0, f))) {
                ...
                if (iblockstate.getMaterial() != Material.AIR || (iblockstate1 = worldIn.getBlockState(blockpos$mutableblockpos1)).getMaterial() != Material.WATER || iblockstate1.getValue(BlockLiquid.LEVEL) != 0 || !worldIn.func_190527_a(Blocks.FROSTED_ICE, blockpos$mutableblockpos1, false, EnumFacing.DOWN, null)) continue;
