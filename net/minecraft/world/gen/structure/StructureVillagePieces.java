...
public class StructureVillagePieces {
    ...
    private static Village generateComponent(Start start, List<StructureComponent> structureComponents, Random rand, int structureMinX, int structureMinY, int structureMinZ, EnumFacing facing, int componentType) {
        int i = StructureVillagePieces.updatePieceWeight(start.structureVillageWeightedPieceList);
        if (i <= 0) {
            return null;
        }
        int j = 0;
        block0: while (j < 5) {
            ...
            for (PieceWeight structurevillagepieces$pieceweight : start.structureVillageWeightedPieceList) {
                ...
                Village structurevillagepieces$village = StructureVillagePieces.findAndCreateComponentFactory(start, structurevillagepieces$pieceweight, structureComponents, rand, structureMinX, structureMinY, structureMinZ, facing, componentType);
    ...
    private static StructureComponent generateAndAddComponent(Start start, List<StructureComponent> structureComponents, Random rand, int structureMinX, int structureMinY, int structureMinZ, EnumFacing facing, int componentType) {
        ...
        if (Math.abs(structureMinX - start.getBoundingBox().minX) <= 112 && Math.abs(structureMinZ - start.getBoundingBox().minZ) <= 112) {
            Village structurecomponent = StructureVillagePieces.generateComponent(start, structureComponents, rand, structureMinX, structureMinY, structureMinZ, facing, componentType + 1);
    ...
    extends Village {
        ...
        public Well(Start start, int type, Random rand, int x, int z) {
            ...
            this.boundingBox = this.getCoordBaseMode().getAxis() == EnumFacing.Axis.Z ? new StructureBoundingBox(x, 64, z, x + 6 - 1, 78, z + 6 - 1) : new StructureBoundingBox(x, 64, z, x + 6 - 1, 78, z + 6 - 1);
        }
    ...
    extends StructureComponent {
        ...
        protected void spawnVillagers(World worldIn, StructureBoundingBox structurebb, int x, int y, int z, int count) {
            if (this.villagersSpawned < count) {
                ...
                int k;
                int j;
                for (int i = this.villagersSpawned; i < count && structurebb.isVecInside(new BlockPos(j = this.getXWithOffset(x + i, z), k = this.getYWithOffset(y), l = this.getZWithOffset(x + i, z))); ++i) {
        ...
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
            ...
            for (int i = this.boundingBox.minX; i <= this.boundingBox.maxX; ++i) {
                block1: for (int j = this.boundingBox.minZ; j <= this.boundingBox.maxZ; ++j) {
                    ...
                    if ((blockpos = worldIn.getTopSolidOrLiquidBlock(blockpos).down()).getY() < worldIn.getSeaLevel()) {
    ...
    extends Village {
        ...
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
            ...
            for (int i = 6; i <= 8; ++i) {
                if (this.getBlockStateFromPos(worldIn, i, 0, -1, structureBoundingBoxIn).getMaterial() != Material.AIR || this.getBlockStateFromPos(worldIn, i, -1, -1, structureBoundingBoxIn).getMaterial() == Material.AIR) continue;
                this.setBlockState(worldIn, iblockstate4, i, 0, -1, structureBoundingBoxIn);
                if (this.getBlockStateFromPos(worldIn, i, -1, -1, structureBoundingBoxIn).getBlock() != Blocks.GRASS_PATH) continue;
