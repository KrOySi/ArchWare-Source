    ...
    public void onLivingUpdate() {
        if (this.world.isRemote) {
            ...
            if (this.isMoving() && this.isInWater()) {
                ...
                for (int i = 0; i < 2; ++i) {
                    this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (this.rand.nextDouble() - 0.5) * (double)this.width - vec3d.xCoord * 1.5, this.posY + this.rand.nextDouble() * (double)this.height - vec3d.yCoord * 1.5, this.posZ + (this.rand.nextDouble() - 0.5) * (double)this.width - vec3d.zCoord * 1.5, 0.0, 0.0, 0.0, new int[0]);
                }
            }
            if (this.hasTargetedEntity()) {
