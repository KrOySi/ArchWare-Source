...
implements IInventory {
    ...
    public int getFirstEmptyStack() {
        for (int i = 0; i < this.mainInventory.size(); ++i) {
            if (!this.mainInventory.get(i).func_190926_b()) continue;
            return i;
        }
        return -1;
    }
    ...
    public int getSlotFor(ItemStack stack) {
        for (int i = 0; i < this.mainInventory.size(); ++i) {
            if (this.mainInventory.get(i).func_190926_b() || !this.stackEqualExact(stack, this.mainInventory.get(i))) continue;
            return i;
        }
        return -1;
    }
    ...
    public int func_194014_c(ItemStack p_194014_1_) {
        for (int i = 0; i < this.mainInventory.size(); ++i) {
            ...
            if (this.mainInventory.get(i).func_190926_b() || !this.stackEqualExact(p_194014_1_, this.mainInventory.get(i)) || this.mainInventory.get(i).isItemDamaged() || itemstack.isItemEnchanted() || itemstack.hasDisplayName()) continue;
            return i;
        }
        return -1;
    }
    ...
    public int getBestHotbarSlot() {
        for (int i = 0; i < 9; ++i) {
            ...
            if (!this.mainInventory.get(j).func_190926_b()) continue;
            return j;
        }
        for (int k = 0; k < 9; ++k) {
    ...
    public NBTTagList writeToNBT(NBTTagList nbtTagListIn) {
        ...
        for (int j = 0; j < this.armorInventory.size(); ++j) {
            if (this.armorInventory.get(j).func_190926_b()) continue;
        ...
        for (int k = 0; k < this.offHandInventory.size(); ++k) {
            if (this.offHandInventory.get(k).func_190926_b()) continue;
