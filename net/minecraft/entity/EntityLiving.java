...
extends EntityLivingBase {
    ...
    protected int getExperiencePoints(EntityPlayer player) {
        if (this.experienceValue > 0) {
            ...
            for (int j = 0; j < this.inventoryArmor.size(); ++j) {
                if (this.inventoryArmor.get(j).func_190926_b() || !(this.inventoryArmorDropChances[j] <= 1.0f)) continue;
                i += 1 + this.rand.nextInt(3);
            }
            for (int k = 0; k < this.inventoryHands.size(); ++k) {
                if (this.inventoryHands.get(k).func_190926_b() || !(this.inventoryHandsDropChances[k] <= 1.0f)) continue;
                i += 1 + this.rand.nextInt(3);
            }
            return i;
        }
    ...
    protected void dropEquipment(boolean wasRecentlyHit, int lootingModifier) {
        for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
            ...
            if (itemstack.func_190926_b() || EnchantmentHelper.func_190939_c(itemstack) || !wasRecentlyHit && !flag || !((double)(this.rand.nextFloat() - (float)lootingModifier * 0.01f) < d0)) continue;
    ...
    protected void setEquipmentBasedOnDifficulty(DifficultyInstance difficulty) {
        if (this.rand.nextFloat() < 0.15f * difficulty.getClampedAdditionalDifficulty()) {
            ...
            for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
                ...
                flag = false;
                if (!itemstack.func_190926_b() || (item = EntityLiving.getArmorByChance(entityequipmentslot, i)) == null) continue;
    ...
    protected void setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty) {
        ...
        for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
            ItemStack itemstack;
            if (entityequipmentslot.getSlotType() != EntityEquipmentSlot.Type.ARMOR || (itemstack = this.getItemStackFromSlot(entityequipmentslot)).func_190926_b() || !(this.rand.nextFloat() < 0.5f * f)) continue;
