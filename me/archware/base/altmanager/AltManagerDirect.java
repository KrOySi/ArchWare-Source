...
extends GuiScreen {
    ...
    public void initGui() {
        ...
        ScaledResolution sr = new ScaledResolution(this.mc);
        this.add = new GuiButton(0, sr.getScaledWidth() / 2 - 60, sr.getScaledHeight() / 2 + 30, 120, 20, "Login");
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 0) {
            AuthenticatorThread authenticator = new AuthenticatorThread(this.login.getText(), this.password.getText());
            authenticator.start();
        }
        if (button.id == 1) {
